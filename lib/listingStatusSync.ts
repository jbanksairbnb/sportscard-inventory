...
// currently in `fromStatus` (single value or array). Returns null on success
// or an error message. Used so a card placed in a live auction or a claimed
// claim-sale lot can't be sold twice from the inventory.
export async function setListingsStatus(
  supabase: SupabaseClient,
  userId: string,
  listingIds: string[],
  status: ListingStatus,
  fromStatus?: ListingStatus | ListingStatus[],
): Promise<string | null> {
...
// from current values rather than from a transition. Idempotent.
//
...
// listing is `active`.
//
// Lock pulls from draft OR active (a freshly-created listing may still be in
// draft when its card is committed to a sale). Unlock pulls only from sold
// to avoid resurrecting removed listings.
