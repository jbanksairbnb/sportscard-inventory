...
async function buildSideBySide(frontUrl: string, backUrl: string | null, bgColor: string = '#ffffff'): Promise<Blob | null> {
  try {
    ...
    // padding visually reads as a tall frame. Side padding stays smaller
    // than the inter-card gap, top/bottom is trimmed further so the
...
// dropped into a comment (each mention typed with a leading @). We surface the
// suggestions pre-sorted by best match, split at this line.
const POST_TAG_LIMIT = 50;

function BidderSuggestionsPanel({ suggestions }: { suggestions: BidderSuggestion[] }) {
  // suggestions arrive pre-sorted by best match (wins → bids → spend).
    ...
    <section className="panel-bordered" style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10, flexWrap: 'wrap' }}>
        ...
        <span style={{ fontSize: 11.5, color: 'var(--ink-soft)', fontStyle: 'italic' }}>
          Sorted by best match — wins first, then bids. Based on past bids on the same player, or any card within ±2 years.
        </span>
        <div style={{ flex: 1 }} />
        <CopyButton
          text={postTagText}
